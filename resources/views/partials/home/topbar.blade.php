...
<div class="w-full bg-gradient-to-l from-[var(--teal)] to-[var(--teal-dark)] text-white text-sm py-2 px-6 items-center justify-between hidden md:flex">
  ...
  <div class="flex items-center gap-3 text-white/80">
    ...
    <div class="flex items-center gap-1" x-data="{ langOpen: false }">
      <button @click="langOpen = !langOpen" @click.outside="langOpen = false" class="flex items-center gap-1 hover:text-[var(--gold)] transition uppercase font-bold">
        <i class="ri-global-line"></i> {{ app()->getLocale() }}
      </button>
      <div x-show="langOpen" x-cloak x-transition class="absolute mt-24 bg-white text-[var(--teal)] rounded-xl shadow-xl border border-gray-100 py-2 w-28 z-50">
        @foreach(['ar' => 'العربية', 'en' => 'English', 'fr' => 'Français'] as $code => $label)
          <a href="{{ route('set-locale', $code) }}" class="block px-4 py-1.5 text-sm hover:bg-gray-50 {{ app()->getLocale() === $code ? 'font-bold text-[var(--gold-dark)]' : '' }}">{{ $label }}</a>
        @endforeach
      </div>
    </div>
